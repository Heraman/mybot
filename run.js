...
async function clientstart() {
    ...
    client.ev.on("messages.upsert", async (chatUpdate, msg) => {
        try {
            ...
            if (newsletterInfo) {
                const { newsletterJid, serverMessageId, newsletterName } = newsletterInfo;
                const responseMessage = `📢 *Newsletter Info*\n\n📌 *JID:* ${newsletterJid}\n🆔 *Server Message ID:* ${serverMessageId}\n📰 *Newsletter Name:* ${newsletterName}`;

                await client.sendMessage(mek.key.remoteJid, { text: responseMessage }, { quoted: mek });
                console.log(`Data newsletter dikirim ke ${mek.key.remoteJid}`);
            }
    ...
    if (connection === 'close') {
        ...
        if (shouldReconnect) {
            clientstart(); // Fungsi untuk menghubungkan ulang
        }
    }
});
